...
exports.findOne = async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({status: false, data: err})
        console.log("Problem in reading user's product")
    }
}
...
exports.addProduct = async(req, res) => {
    ...
    try {
        ...
        res.status(200).json({ status: true, data: results})
        console.log("Succcess in saving product")
    }catch(err) {
        res.status(400).json({status: false, data: err})
...
exports.updateProduct = async (req, res) => {
    ...
    try {
            const result = await User.updateOne(
                { username: username, "products.product": product_id},
                {
                    $set: {
            ...
            console.log("Succcess in updating product")
    } catch(err) {
        res.status(400).json({status: false, data: err})
...
exports.deleteProduct = async (req, res) => {
    ...
    try{
        const result = await User.updateOne(
            ...
            res.status(200).json({ status: true, data: result})
            console.log("Succcess in deleting product")
    } catch(err) {
        res.status(400).json({status: false, data: err})
